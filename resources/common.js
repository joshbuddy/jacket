...
if(
(
(navigator.userAgent.indexOf('Opera') == -1 && navigator.userAgent.indexOf('Safari') == -1)
|| navigator.userAgent.indexOf('Chrome') != -1
)

&& !(navigator.userAgent.indexOf('Firefox') > 0 && navigator.userAgent.indexOf('Mac') > 0)
){
...
{
e = RGraph.FixEventObject(document.all ? event : e);if(e.target.parentNode.__canvas__.__object__.Get('chart.zoom.mode') == 'window'){
return RGraph.ZoomWindow(e);}
var canvas = e.target.__canvas__;var context = canvas.getContext('2d');var obj = canvas.__object__;var tmp = canvas;var x = 0;var y = 0;var factor = obj.Get('chart.zoom.factor') - 1;while (tmp != document.body){
...
canvas.onmousemove = function (e)
{
var obj = e.target.__object__;var canvas = obj.canvas;var context = obj.context;var coords = RGraph.getMouseXY(e);if(!RGraph.Registry.Get('chart.zoomed.div')){
div = document.createElement('div');div.style.width = obj.Get('chart.zoom.thumbnail.width') + 'px';div.style.height = obj.Get('chart.zoom.thumbnail.height') + 'px';div.style.border = '2px dashed gray';div.style.position = 'absolute';div.style.overflow = 'hidden';div.style.backgroundColor = 'white';div.style.left = '-1000px';div.style.top = '-1000px';div.style.MozBorderRadius = '5px';div.style.WebkitBorderRadius = '5px';if(obj.Get('chart.zoom.shadow')){
div.style.MozBoxShadow = 'rgba(0,0,0,0.5) 3px 3px 3px';div.style.WebkitBoxShadow = 'rgba(0,0,0,0.5) 3px 3px 3px';}
div.style.opacity = 0.2;div.__object__ = obj;document.body.appendChild(div);var img = document.createElement('img');img.width = obj.canvas.width * obj.Get('chart.zoom.factor');img.height = obj.canvas.height * obj.Get('chart.zoom.factor');img.style.position = 'absolute';img.style.backgroundColor = 'white';img.src = obj.canvas.toDataURL();div.appendChild(img);                
RGraph.Registry.Set('chart.zoomed.div', div);RGraph.Registry.Set('chart.zoomed.img', img);setTimeout("RGraph.Registry.Get('chart.zoomed.div').__object__.canvas.onmouseover()", 5);} else {
...
var x = 0;var y = 0;var o = obj.canvas;while (o != document.body){
x += o.offsetLeft;y += o.offsetTop;o = o.offsetParent;}
var offset = 7;div.style.left = (e.pageX - obj.Get('chart.zoom.thumbnail.width') - offset) + 'px';div.style.top = (e.pageY -  obj.Get('chart.zoom.thumbnail.height') - offset) + 'px';var l = (obj.Get('chart.zoom.thumbnail.width') / 2) - (coords[0] * obj.Get('chart.zoom.factor'));var t = (obj.Get('chart.zoom.thumbnail.height') / 2) - (coords[1] * obj.Get('chart.zoom.factor'));img.style.left = (l + ((obj.Get('chart.zoom.thumbnail.width') / 2) * obj.Get('chart.zoom.factor'))) + 'px';img.style.top = (t + ((obj.Get('chart.zoom.thumbnail.height') / 2) * obj.Get('chart.zoom.factor'))) + 'px';}
canvas.onmouseover = function (e)
{
var div = RGraph.Registry.Get('chart.zoomed.div');if(!div) return;var targetWidth = parseInt(div.style.width);var targetHeight = parseInt(div.style.height);RGraph.Registry.Get('chart.zoomed.div').style.width = 0;RGraph.Registry.Get('chart.zoomed.div').style.height = 0;if(RGraph.Registry.Get('chart.zoomed.div').__object__.Get('chart.zoom.fade.in')){
